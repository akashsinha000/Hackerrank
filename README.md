...
The solution includes an OpenAI-compatible agentic advisor. It receives the request, user profile, relevant messages, and deterministic financial baseline, then proposes an explanation. The deterministic simulator remains the safety authority and rejects proposals that do not match the validated payment method.

Configure it with environment variables before running:

```powershell
$env:BUY_WAIT_LLM_API_KEY = "your-key"
$env:BUY_WAIT_LLM_MODEL = "gpt-4o-mini"
$env:BUY_WAIT_LLM_ENDPOINT = "https://api.openai.com/v1/chat/completions"
python code/main.py
```

If no API key is configured, the program runs deterministically with zero model calls. Usage details are written to `code/evaluation/usage_report.md` after every run.
